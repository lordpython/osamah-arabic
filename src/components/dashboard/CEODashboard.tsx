...
export default function CEODashboard() {
  ...
  const { data: totalRevenue } = useSupabaseQuery<number>('accounting_entries', async (supabase) => {
    const { data, error } = await supabase.from('accounting_entries').select('amount').eq('type', 'income');
  ...
  const { data: totalDrivers } = useSupabaseQuery<number>('drivers', async (supabase) => {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <MetricCard title="Total Revenue" value={totalRevenue || 0} icon="💰" trend={0} format="currency" />
        <MetricCard title="Active Drivers" value={totalDrivers || 0} icon="🚗" trend={0} />
        <MetricCard title="Fleet Utilization" value={85} icon="📈" trend={5} format="number" />
      </div>
